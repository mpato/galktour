...
public class Feature implements JavaScriptComponent
{
  ...
  public JavaScriptSnippet toJavaScript()
  {
    ...
    snippet.addInitialization("style = new ol.style.Style({ image: new ol.style.Icon(/** @type {olx.style.IconOptions} */ ({anchor: [0.5, 46], anchorXUnits: 'fraction'," +
                              "anchorYUnits: 'pixels',opacity: 0.75,src: 'http://openlayers.org/en/v3.2.0/examples/data/icon.png'}))})");

    snippet.addInitialization("\nicon = new ol.Feature({geometry: new ol.geom.Point([0, 0]),name: 'Null Island',population: 4000,rainfall: 500})");
    snippet.addInitialization("\nicon.setStyle(style);");
    snippet.add("icon");
    return snippet;
  }
